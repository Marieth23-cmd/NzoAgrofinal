...
export default function CategoriaGraos() {
    interface Produto {
      id_produtos: number,
      nome: string,
      preco: number,
      ...
      try {
        ...
        // Cenário 2: Se o usuário selecionou província mas não tipo específico
          ...
          if (produtosPorProvincia.length > 0) {
            ...
            if (produtosFiltrados.length === 0 && precoFiltroInput) {
              ...
              sugestoes.mensagem = `Em ${provinciaFiltroInput} ${precoFiltroInput ? `na faixa ${traduzirFaixaPreco(precoFiltroInput)}` : ''}, temos estas Grãos:`
            }
          }
        }
        ...
        // Cenário 3: Se o usuário só selecionou faixa de preço
          ...
          if (produtosPorPreco.length > 0) {
            ...
            sugestoes.provincias = provinciasDisponiveis.slice(0, 3)
            sugestoes.tipos = tiposDisponiveis.slice(0, 3)
            sugestoes.mensagem = `Na faixa ${traduzirFaixaPreco(precoFiltroInput)}, temos Grãos em:`
          }
        }
 ...
 useEffect(() => {
    ...
    async function carregarProdutosParaSelects() {
      try {
        const produtosRecebidos = await buscarProdutosPorCategoria("Graos", {})
        if (isMounted) {
  ...
  return (
    <main>
      <Head>
        <title>Categoria Grãos</title>
      </Head>
      <Navbar />

      <div className="mb-20 mt-[52%] lg:mt-[18%]" role="main">
        <h1 className="text-center my-6 text-[1.5rem] lg:text-[2rem] sm:text-[1.75rem] md:text-[2rem] font-bold text-marieth" aria-label="Tubérculos e Raízes">Grãos</h1>
...
>
        ...
        {/* Seção de produtos */}
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mt-8">
